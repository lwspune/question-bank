...
> **Re-measured 2026-09-21 (12 cores, warm caches) — and two stages changed.** typecheck 9 s · lint 4 s · docs:budget 1 s · notes:latex 2 s · **notes:lint 91 s** (it grows with the bank) · **test 147 s at `maxForks: 5`** (was 317 s at 2; measured 2/4/6/8 → 317/194/120 s green, 8 fails) · build 4-9 min unchanged. The static checks are **16 s** of the gate. Two changes: (1) the fork cap is 5 — its old "ONE shared prod project" rationale predated the test-project split; the real walls are Supabase Auth's `/auth/v1/token` limit (**150 per 5 min PER IP**, shared by a gate re-run a minute after a red one) and test-project contention. A rate-limited sign-in used to surface as a fake `42501` RLS failure because 41 sign-ins discarded the error — `mustSignIn` now throws with the reason, so read the FIRST error, not the RLS one. (2) **notes:lint is conditional** — the hook picks one of four `prepush*` scripts (build × notes:lint) via `scripts/needs-build.ts` + `scripts/needs-notes-lint.ts`; CI mirrors both. A push touching nothing under `src/app/notes`, `src/lib/notes`, the lint, `supabase/` or deps skips it; run it by hand after an ingest or taxonomy rename, which no diff can see. Typical no-build push: **~2m45s** (was ~7 min). Runs at 5 forks, spaced 5 min apart: #1 146.7 s · #2 130.1 s, both green.

...
5. **Formula enrichment** (notes edit, *conditional*): if step 2 flagged empty-`formula.latex` concepts, **TRIAGE FIRST — expect most flags to be noise** (the probe can't tell a recallable formula from a derivation step). The decision rule:
   - **ENRICH** only a concept whose prose teaches a GENUINE RECALLABLE FORMULA / IDENTITY — a memorised equation (`f'(x)=\lim…`, `\tan^{-1}\frac{2x}{1-x^2}=2\tan^{-1}x`, `S_m=n,S_n=m⇒S_{m+n}=-(m+n)`, `\prod a_i = M^{2m-1}`).
   - **SKIP** a TECHNIQUE / METHOD / CRITERION with no recallable equation (read-the-symmetric-form, log-differentiate, substitute-the-point, AGP shift-subtract, collinearity test). Those are correctly computation-only — leave `formula.latex` empty.
   - **When in doubt, SKIP** (don't pad a quiz with derivation-step noise).
   
   Then **append** each genuine formula to the concept's `formula.latex` (append-only — preserves piece indices/fingerprints; safe on EMPTY concepts, watch the single→bundle `auto`-flip on non-empty ones) → re-harvest → author only the new pieces → delete + re-assemble that chapter's formula quiz. **(Bucket 2, 2026-06-10: of 16 flagged Wave-2 concepts only 4 were genuine — 75% were techniques; 3D-Geometry's 5 flags were ALL techniques → 0 enriched. The triage IS the work.)**
...
**ALL 30 noted NDA Maths chapters + HP complete (Waves 1-7) — NDA Maths daily quiz is DONE** (the prioritized-subset campaign: parallel sub-agents author distractors + notes enrichment — one per chapter, heavy chapters split by subtopic; the main agent harvests/syncs/verifies/assembles/pushes serially). Wave 3 (2026-06-12) = limits-continuity, permutation-combination, application-of-derivatives, complex-numbers — 16 quizzes / 225 q (all 4 needed formula+trap from scratch). Wave 4 (2026-06-12) = the next 4 by weight: sets-relations, definite-integration, quadratic-equations, differential-equations — 17 quizzes / 251 q (these were RICHER — all 4 already had trap callouts, QE coverage was fully clean; all 12 agent-predicted verify keys matched first run incl. def-int's re-indexed FTC/King's pieces; differential-equations' formula+trap were topped up from 8/11 to 13/13 post-hoc by exposing genuine recall items — exact differentials, linear-in-x IF, Bernoulli — that the conservative first triage had skipped). Wave 5 (2026-06-12) = binomial-theorem, properties-of-triangle, indefinite-integration, conics — 12 quizzes / 172 q; all 12 predicted verify keys matched first run. **Key Wave-5 pattern: the harvest's "MCQ-clean" count conflated formula-piece atoms with computation, so computation was over-counted** — several chapters actually had near-zero practiceSet (indefinite-integration 0, properties 2, binomial-theorem 2), so the agents TOPPED UP computation with genuine practiceSet items (+12/+10/+11) to clear the floor (also enriches the student notes); Properties/Conics also needed heavy trap-callout authoring (+9/+8, they had 3/4). Conics computation (3 practiceSet) genuinely parked — it's a formula/trap chapter. (+ binomial-distribution shipped as a single chapter the same day — computation topped up 10→14.) Wave 6 (2026-06-12) = inverse-trigonometry, trigonometric-equations, circles, logarithms — 14 quizzes / 189 q; all 12 predicted verify keys matched first run. This wave leaned HEAVILY on the Wave-5 top-up lesson: most chapters had near-zero practiceSet (inverse-trig 2, trig-eq 2, circles 7) so computation was topped up with genuine practice (+22/+10/+10), and Inverse-Trig/Trig-Eq needed +9/+10 trap callouts (had 4/2). Every theme cleared the 12-floor on all 4 chapters. **27 of 30 NDA Maths chapters now quizzed — only Applications of Integration, Height & Distance, Binary Numbers remain.** Wave 7 (2026-06-12, the finisher) = those final 3 — 11 quizzes / 145 q; all 9 predicted verify keys matched first run. Same top-up pattern (applications/H&D had 5/7 practiceSet → topped up; binary is computation-heavy at 39). Binary formula PARKED at 8 by design (few genuine recall formulas). **NDA Maths daily quiz is now COMPLETE: all 30 noted chapters across all harvestable themes (~120 quizzes built).** The next frontier is the other subjects — NDA Chemistry (11 notes chapters), Physics (9), Biology (8), all authored quiz-ready, plus the 2 old NDA Physics + 1 MHT-CET. All quizzes are snapshot-backed (0035).
...
| nda-maths / **binomial-distribution** | ~43 | 3 | 🟠 **2026-06-12** — computation 1 (14) + formula 1 (17; bundled `formula.latex` split into 17 pieces) + trap 1 (12). Coverage was already clean (14 concepts, 12 traps). Computation first PARKED at 10 (<12); **topped up to 14** by adding 4 genuine practiceSet items (complement-at-least-one ×2, mean-variance-relation ×2) to concepts that had a worked example but no practice — improves the student notes too. |
| nda-maths / **differential-equations** | ~57 | 4 | 🔵 **Wave 4 (2026-06-12)** — computation 2 (31 q) + formula 1 (13) + trap 1 (13). Formula/trap were initially PARKED at 8/11 (<12); **topped up to 13 each** by exposing genuine recall items the first triage skipped — exact differentials (`d(xy)`/`d(x/y)`/`½d(x²+y²)`), linear-in-x IF, Bernoulli `v=y^{1-n}` (formula) + 2 sign/exponent traps. Re-derivation also fixed a garbled notes answer (circles-on-x-axis order). |
| nda-maths / **binomial-theorem** | ~47 | 3 | 🟢 **Wave 5 (2026-06-12)** — computation 1 (13; +11 practiceSet, chapter shipped with only 2) + formula 1 (18) + trap 1 (13). Coverage clean; formula 3 leftover parked. |
| nda-maths / **properties-of-triangle** | ~42 | 3 | 🟢 **Wave 5 (2026-06-12)** — computation 1 (12; +10 practiceSet, chapter had 2) + formula 1 (18) + trap 1 (12; +9 callouts — chapter had only 3). |
| nda-maths / **indefinite-integration** | ~60 | 4 | 🟢 **Wave 5 (2026-06-12)** — computation 1 (12; +12 practiceSet, chapter had 0) + formula 2 (26; enriched antiderivative-properties +2) + trap 1 (18; 22 seeds, 4 parked). |
| nda-maths / **conics** | ~33 | 2 | 🟢 **Wave 5 (2026-06-12)** — formula 1 (18; bundle-split, formula-rich) + trap 1 (12; +8 callouts — chapter had 4). computation 3 PARKED (chapter has only 3 practiceSet; conics is formula/trap-heavy). |
| nda-maths / **inverse-trigonometry** | ~51 | 4 | 🔵 **Wave 6 (2026-06-12)** — computation 2 (24; chapter had only 2 practiceSet → +22) + formula 1 (14) + trap 1 (13; +9 callouts, had 4). |
| nda-maths / **trigonometric-equations** | ~38 | 3 | 🔵 **Wave 6 (2026-06-12)** — computation 1 (12; had 2 → +10) + formula 1 (14; enriched +3 general-solution formulas; 2 auto parked) + trap 1 (12; +10 callouts, had 2). The thinnest Wave-6 chapter, still cleared all 3 floors. |
| nda-maths / **circles** | ~61 | 4 | 🔵 **Wave 6 (2026-06-12)** — computation 1 (17; had 7 → +10) + formula 2 (24) + trap 1 (18; 20 seeds, 2 parked). Coverage was clean. |
| nda-maths / **logarithms** | ~41 | 3 | 🔵 **Wave 6 (2026-06-12)** — computation 1 (13; had 11 → +2) + formula 1 (15) + trap 1 (13; +3 callouts, had 10). |
| nda-maths / **applications-of-integration** | ~42 | 3 | 🟣 **Wave 7 (2026-06-12)** — computation 1 (17; had 5 → +11) + formula 1 (13) + trap 1 (12; +2 callouts, had 10). |
| nda-maths / **height-distance** | ~57 | 4 | 🟣 **Wave 7 (2026-06-12)** — computation 1 (13; had 7 → +7) + formula 2 (27; application-rich, each sub-model has its own relation) + trap 1 (17). Coverage was clean. |
| nda-maths / **binary-numbers** | ~61 | 4 | 🟣 **Wave 7 (2026-06-12)** — computation 3 (39; computation-heavy chapter) + trap 1 (13; +2 callouts, had 11). formula 8 PARKED (<12; binary has few genuine recall formulas — by design). |
| **NDA Maths: 30/30 noted chapters DONE** (Linear Inequalities 5 q has no notes, below the gate). _Frontier: NDA Chemistry (11), Physics (9), Biology (8) notes + NDA Physics (2 old), MHT-CET (1)._ | — | — | Not harvested |

...
│   ├── browse/                            page.tsx + loading.tsx + Hero (animated fade-in-up) + FilterBar (incl. pyqYears chips) + MobileFilters (Sheet) + ActiveFilterChips (dismissible chip rail above results, derived from buildActiveChips pure helper) + QuestionList (groups consecutive set-siblings under a passage banner via groupBySet; standalone questions render as plain cards) + QuestionCard (with Add-to-paper toggle; takes hideContext to suppress per-card Context when inside a set group) + Pagination + DownloadDialog (filter-mode / cart-mode toggle) + CartPill (floating pill with pill-in entrance + count-pulse on add + Sheet panel: per-row remove, sort toggle Added-order/By-chapter, Clear, Download)
...
│   └── api/export/                        POST → ZIP of Question Paper + Answer Key. Body is either { filters, options } or { questionIds, options } (mutually exclusive). Cart-mode preserves caller's id order so the printed paper matches the chosen sort.
├── lib/
...
│   ├── questions/{filters,query,edit,applyEdit,deleteQuestion,dirty}.ts   browse filters ↔ URL · Supabase query builder (queryQuestions sorts `created_at DESC, source_row ASC, id ASC` so within-upload Excel order is preserved; queryQuestionsByIds + queryQuestionPreviewsByIds for cart-mode preserve caller's id order) · zod edit schema + hash · DB-side edit application (fans out context change to set siblings sharing set_id) · admin delete (cascade + storage cleanup, discriminated-union return) · pure-function form-state diff for edit page
...
│   ├── upload/{parser,validate,hash,taxonomy,commit}.ts   upload pipeline (pure → DB). parser reads optional Excel columns Q (question_number), Course, Set; commit stamps upload_job_id + pyq_year/pyq_month/pyq_note + question_number + set_id (`<uploadJobId>:<setLabel>`, scoped per upload) onto every inserted row. Taxonomy auto-create runs in two parallel pre-resolve passes (unique chapter keys, then unique subtopic keys) before the row loop; resolveChapter/resolveSubtopic catch 23505 and re-find the winner's id so concurrent commits adopt rather than fail.
...
│   ├── text/normalizeNewlines.ts          pure helper. Masks math zones (`\(…\)`, `\[…\]`, `$…$`, `$$…$$`) to opaque placeholders, replaces literal `\n` (backslash+n) outside math with real newlines, restores math zones verbatim. Idempotent. Called from upload parser + edit validator so the DB never carries literal `\n` past the boundary; LaTeX commands like `\neq`, `\nu`, `\nabla` inside math are untouched.
...
├── components/
│   ├── AppHeader.tsx + UserMenu.tsx       sticky 56px header (logo + truncated org name visible on mobile + avatar trigger). UserMenu uses Radix Popover (focus-trap + escape + click-outside handled natively); items are dark-mode toggle + sign-out.
...
tests/                                     48 .test.ts files, ~330 tests
...
- **Word export = ZIP of two files.** First version was a single .docx with a 3-way layout toggle. User asked to split: a Question Paper distributable without leaking answers, and a separate Answer Key. One ZIP keeps the bundle obvious.

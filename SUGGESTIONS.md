...
|---|---|---|
| **2 errors in the EXISTING PUBLIC MH-HSC-12 Maths *textbook* (practice) corpus**, surfaced 2026-08-13 while authoring board-PYQ solutions against that corpus as the reuse source (`scripts/mh-hsc-12-pyq`). Not found by any probe — found because a PYQ asked the same thing and the twin's answer disagreed. | (1) **Trigonometric Functions** — the shipped solution for the triangle \(a=13,b=14,c=15\) prints **cos A = 33/65**, which is actually cos B; the correct value is **3/5**. Its own parts (ii)–(vi) are internally consistent with 3/5, so only part (i) is wrong — i.e. a copy slip, not a wrong method. (2) **Trigonometric Functions**, Properties of Inverse Trig — a row states \(2\tan^{-1}(-1/3) + \cos^{-1}(3/5) = \pi/2\), which is **false**; the identity holds for \(+1/3\). | **Identified — NOT actioned.** Shipped work, so per the learning-propagation protocol this needs a 360 + explicit permission before any edit. Both were *reported* by the authoring pass and deliberately not replicated: the board-PYQ answers computed from the correct values (sec A = 5/3 from cos A = 3/5) and cite the twin only for method. **Before acting, re-derive both against live data** — the NDA Maths ledger row above went stale and would have "corrected" two already-correct rows. Scope is 2 rows in one chapter; blast radius is nil (no `/notes` chapter or quiz atom reads them); the fix is a solution-text edit, which does NOT move `content_hash`. |

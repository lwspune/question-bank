...
| **Figure-as-OPTION questions committed as TEXT descriptions can give away the answer + are lossy** — surfaced reviewing the MHT-CET 2025-April ingest ([[figure-image-acquisition]]) | The ~7 PUBLIC MHT-CET 2025-April "which structure is X" rows ingested with `optionImages` text-describe instead of attached figures. Worst: **Q93** (`MHT_CET_2025_Apr_20_S1.docx`, "identify the structural formula of phloroglucinol") — option A text literally reads "…— phloroglucinol", **naming the answer**. Lower-risk: S1 Q79/Q89, S2 Q66/Q121, S2-20Apr Q74/Q79 (describe branching/structure, closer to faithful). Also their `content_hash` is weak (placeholder option text). | **Identified — awaiting permission.** 360 first; Q93 highest priority. Fix = re-attach the original option figures (re-run `scripts/mhtcet/extract.ts` on the source docx → the embedded PNGs are in the gitignored `out/media/`; set `optionFigures`) + neutralise the giveaway option text + fold the option image-hash into `content_hash`. Delete-then-recommit the affected rows (hash changes). |

---

## 2026-06-26

### Make the ingestion figure-OPTION policy "attach, don't describe" + harden the image-option content_hash

...
### `THRESHOLDS.dbSizeCapBytes` is calibrated to the FREE plan; the project is on Pro — **status: identified, awaiting-permission**

**What.** `src/lib/dbhealth/flags.ts` carries `dbSizeCapBytes: 500 * 1024 * 1024` commented *"Supabase free plan. Confirm on the dashboard if the plan ever changes."* The project is on **Pro** (CLAUDE.md "Live infra"), where 8 GB is included and disk grows automatically.

**Why it matters, with a date.** The rule warns at 80% of that cap = 400 MB. The database measured **200 MB on 2026-08-19** and grows ~3 MB/day, so it will start emitting a **false** "database is 80% of plan cap" warning in roughly two months — on a database with ~40x the headroom it thinks it has. A monitor that cries wolf gets ignored, which is this tracker's own stated design constraint, so the cost is not cosmetic: it degrades every other rule in the same report.

**Found while** fixing the *other* miscalibrated threshold in the same `THRESHOLDS` block (2026-08-19, migrations 0080/0081). Deliberately not changed in passing — it is shipped code and a different rule, and quietly widening scope mid-fix is how a reviewable change becomes an unreviewable one.

**360 still owed** before touching it: scope (one constant + its comment + the `/dashboard/health` "plan cap" note that renders it), blast radius (report-only; also drawn as a Meter on the health page), does-it-really-apply (yes — verified plan, verified current size), risk/reversibility (trivial, one constant), cost (minutes), recommendation (do — but decide the right number first: Pro's *included* 8 GB is not a hard cap, so a cap-shaped gauge may be the wrong shape and a growth-rate rule may fit better).

**Same shape as** [[project-docs-staleness]]: a plan upgrade leaves no trace in the repo, so every number derived from the plan silently rots. The egress figures in the 2026-08-05 entry had already cost an hour for exactly this reason.

| Learning | Existing-work candidate | Status (2026-06-10) |
|---|---|---|
| **The 12 `/guide/nda-maths/principles/[slug]` deep-dive pages print `●` (SSG) in the build's route table and produce ZERO `.html` files** — every prerender bails, and has been bailing since the route shipped. Surfaced 2026-08-19 while verifying the modulus principle split: the gate passed, the route table listed all 12 slugs under `●`, and `find .next/server/app -path '*principles*' -name '*.html'` returned only the index page. Cause is [`[slug]/page.tsx:52`](src/app/guide/nda-maths/principles/[slug]/page.tsx#L52) calling `createSupabaseServerClient()`, which reads cookies and therefore forces dynamic rendering despite `generateStaticParams`. This is exactly the trap [[shell-component-decaches-site]] documents ("a route still prints `●` even when every prerender bailed; the only reliable probe is the filesystem") — recorded there against `AppHeader`, now found a second time on a page-level loader. | The page's only DB reads are `loadPrincipleQuestionIds` (tag table) and `loadWorkedExamples` (PUBLIC questions), neither of which needs a user session — so the anon client is very likely sufficient, the same swap the five `/guide/nda-maths` sub-pages took on 2026-08-13 for the identical reason. **Verify before assuming**: confirm no PRIVATE row can reach these pages (the 2026-08-13 change was safe only because 0 of 488 principle-tagged questions are PRIVATE — re-measure, since the tag table has since changed). Acceptance is filesystem-based, not the route table: 12 new `.html` files on disk, and a grep of the emitted `.html`/`.rsc` for emails / org names / authed-only controls per the cache-safety rule. | **Identified 2026-08-19.** Not applied with the principle-split work because `[slug]/page.tsx` is shipped code and the learning-propagation protocol gates a retroactive fix on explicit permission — and because a caching change should not ride along inside a content commit. Blast radius is one import + one call site; the risk is serving one cached copy to everyone, which is why the acceptance check is artifact-grep rather than reasoning. Awaiting permission. |

...
|---|---|---|
| **A solution that ASSERTS instead of DERIVING is a CORRECTNESS probe, not a readability one — measured 2026-08-25, and the ratio is the finding.** Sweeping NDA Maths for phrases that carry a conclusion with no executed step (`verified numerically`, `evaluating gives`, `it can be shown`, `the printed solution`, `by an appropriate`) flagged **79 of 5,347 PUBLIC solutions**. Rewriting them produced **62 rewrites, 2 legitimate keeps, and 15 REPORTs where the keyed answer could not be reached from the stem at all** — a 19% defect rate among flagged rows. Nearly every REPORT is a **mis-transcribed stem under a CORRECT key**, and the hand-waving was the symptom rather than the disease: the original author could not derive the printed answer from the corrupted stem either, so they named a method and asserted its result. **RULE 4's blind re-derivation cannot see this class** — the key is right, so the row simply AGREEs. One row proves why numeric spot-checks are banned as justification: the true and transcribed relations intersect at exactly one point, and that point is the obvious angle to test (45 degrees), so a check there agrees by coincidence and falsely confirms the wrong stem. | **14 REPORTed rows still need source adjudication against the printed papers** (the 15th, NDA 2024-II Q32, was source-verified and fixed this session). Each is a stem or key change, several are PYQs live in published mocks with student answers already recorded, and an agent's reconstruction of the intended stem is a HYPOTHESIS: on Q32 the agent inferred the quantity should be `xy+yz+zx`, while the printed paper showed the quantity was always `x+y+z` and it was the RELATION that was mis-transcribed (quotients, not products). Plausible and wrong — so none of these may be applied on a derivation alone. Sharpest of the 14: **Q91 (2017-Sep, Limits, `f(x)=[x]^2-[x^2]`)** where the agent's integer-by-integer table gives option D against the stored B — a claimed WRONG KEY rather than a wrong stem, in a mock with 2 answers. Beyond NDA Maths the same markers are unswept in every other subject and exam. | **Identified 2026-08-25; the 62 rewrites are DONE and recorded** in `question_reviews` as `solution_rewritten`/`solution_audit` under run `handwave:2026-08-25-handwave-nda-maths`, all hash-neutral (a solution edit does not touch `content_hash`), all render-verified on web and Word. The 14 REPORTs are logged here and NOT applied — they are shipped PUBLIC rows and several are live in mocks, so they go through source verification one at a time, the way Q32 did. Tooling is ready: `dump-handwave.ts` -> rewriters -> `merge-handwave-rewrites.ts` -> `apply-solution-rewrites.ts` -> `record-solution-rewrites.ts`. |

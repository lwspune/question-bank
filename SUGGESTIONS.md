...
- **11 NEET figures are still clipped, and each needs anchors placed BY HAND.** After sweeping all 8 NEET papers (175 figures), these resisted the automated pass: **2023 Q21, Q37, Q40, Q66, Q92, Q95, Q169** · **2021 Q22, Q48, Q138** · **2026 Q35**. Do NOT re-run `auto_anchor` on them — it was tried and reverted. On 2023 it merged stem + figure + options into one "block" (that paper is a continuous-flow document with tight line spacing) and produced full-page-width crops that swallowed the option list and, on Q169, the **"Answer (2)" line** — strictly worse than the clipping it was fixing. 2021 Q22 is a **probe false positive**: the ink continuing below its bbox is the option row, so the original crop was already correct. 2026 Q35's top anchor lands inside content and snapCrop rightly refuses. The reliable procedure is the one used on the 22 that did get fixed: `profile_page` the column → place `top`/`bottom` in real whitespace gaps → `snap-crop` → confirm with BOTH montages.

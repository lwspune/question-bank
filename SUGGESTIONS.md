...
| **1 pre-existing internal contradiction in the EXISTING PUBLIC NDA Geography bank** (two near-identical Ring-of-Fire rows with opposite keys), surfaced by the semantic-dedup scan of the "Interior of Earth & POP 50 Q" LWS test (2026-07-25) — the paper itself is all net-new (49 new / 1 maybe / 0 dup), so this is a bank-internal issue, not paper-derived | `d0b9971a` (Ring of Fire — {1. convergent plate boundaries, 2. seismic & volcanic, 3. deep ocean trenches}) keyed **D (1,2,3)** vs `21fbe2f1` (same cluster, "convergence plate boundaries / active seismic & volcanic / deep trench") keyed **B (2 and 3 only)**. Statement 1 (Ring of Fire ≈ convergent boundaries) is NCERT-standard-correct, so **D is defensible and `21fbe2f1`'s B is the suspected wrong-key.** | **Identified — awaiting permission.** A `is_correct` move B→D on `21fbe2f1` + solution reconcile + `content_hash` recompute via the real helper (collision-guarded). Confirm both rows' exact stems/options against a source before flipping per [[audit-probe-symmetry]]; they may be legitimately distinct (e.g. one qualifies "convergent" differently). |

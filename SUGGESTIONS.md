...
### Decide the 4 non-student accounts on the mailable roster (before the first real campaign send)

The mock-recommendation dry run (2026-07-15) surfaced 4 roster entries that aren't students: a junk signup (31 May, never attempted), a partner/business address, the owner's own personal account, and — the structurally interesting one — **the former APJ-org account, which became org-less when the APJ org was deleted during the 2026-07-15 branch re-tenanting, so the roster silently reclassified it as a student**. `deriveStudents` defines a student as *any `auth.users` row with no `org_members` row*: correct for the `/dashboard/students` roster, but that definition is now also the **mailing list**. (The specific addresses are deliberately NOT recorded here — this repo is public and they're personal data; they're in the private `project-mock-recommendation-email` memory note and reproducible any time via `npx tsx scripts/email/send-next-mock.ts`, which sends nothing.) Two leaked `@test.invalid` billing fixtures were also picked up but are now structurally excluded by `isUndeliverable()`.

**Why:** the APJ case is the real finding — **deleting an org silently promotes its members onto the student roster**, and nothing flags it. That recurs on the next re-tenanting, and the roster is no longer just a display surface. The other three are cosmetic until real sends start, at which point they're wasted volume and (for a partner address) mildly embarrassing.

**How to apply:** suppress per-account with `email_opt_out=true` via the real mechanism (as the QA test account was) — never a hardcoded skip-list in `recommend.ts`, which would rot and hides intent from the DB. Structurally, consider whether `deriveStudents` should also exclude `platform_admins`, and whether deleting an org should re-home or tombstone its members rather than dropping them into the student pool.

**Related hygiene:** roster/email work reads real addresses out of `auth.users`. Keep them out of tracked docs — describe accounts by ROLE (junk signup / partner / former-org staff). This entry originally named all four and was caught pre-commit.

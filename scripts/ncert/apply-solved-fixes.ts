 ...
 *
 *   npx tsx scripts/ncert/apply-solved-fixes.ts <chapterId>          # dry-run
 *   npx tsx scripts/ncert/apply-solved-fixes.ts <chapterId> --apply  # write
 *
 * WHY THIS SCRIPT EXISTS AT ALL. A `solved` row ships the BOOK's own worked
 * solution, committed inline from the transcription band — so `apply-solutions.ts`
 * never touches it (that one only fills rows where `solution IS NULL`). When the
 * book's printed derivation is not merely ugly but arithmetically WRONG,
 * preserving it verbatim ships a model answer that TEACHES the error.
 *
 * The shipped precedent across three pipelines is the same: correct the
 * derivation AND carry an errata bracket naming the defect. This does the first
 * half; `apply-errata.ts` does the second and must run AFTER — and, per the
 * standing rule, `apply-errata` must be a chapter's LAST write.
 *
 * WHEN NOT TO USE IT. Only where the correct value follows unambiguously from
 * the book's OWN printed inputs. If you cannot reconstruct what the book meant,
 * preserve the line verbatim and bracket it instead — inventing a "correction"
 * is worse than shipping a defect you have named, because the bracket at least
 * tells the reader what to distrust.
 *
 ...
 * a silent no-op that reads as success, which has bitten the mh-hsc-12-pyq
 * pipeline; and a fix whose `find` equals its `replace` is refused too, since
 * that is exactly what a shell-mangled needle looks like.
 *
 * Writes BOTH the live row and the transcription band that carries the ref, so a
 * later re-commit cannot revert the repair.
 */
...
async function main() {
  ...
  for (const f of fixes) {
    ...
    // A control character or a doubled backslash is the signature of a needle
    // authored through a shell heredoc; refuse rather than write it.
    for (const [label, v] of [["find", f.find], ["replace", f.replace]] as const) {
      if (/[\x00-\x08\x0b\x0c\x0e-\x1f]/.test(v))  {
        throw new Error(`${f.ref}: control character in "${label}" — author this through the editor, not a heredoc`);
      }
    }
  ...
  // Never mirror into a generated artifact — only into a transcription band.
  const SCRATCH =
    /\.(errata|tosolve|topaper|solutions|mcq-blind|mcq-verify|crosscheck|review|xcheck|fig|figtext|anchors|diagram-specs|solution-images|solved-fixes|key)\.json$/;
  ...
  for (const f of fixes) {
    ...
    for (const file of jsonFiles) {
      ...
      const hit = arr.find((r) => r && r.ref === f.ref);

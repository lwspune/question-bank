 ...
 * bboxes, which clipped figures and leaked solution text. snapCrop bounds the
 * actual INK inside the anchor band on the RENDERED page, so it works the same on
 * NCERT's scanned pages as on vector art — it was built for the NEET scans.
 *
 *   npx tsx scripts/ncert/snap-crop.ts <chapterId>          # derive + report (dry: writes nothing)
 *   npx tsx scripts/ncert/snap-crop.ts <chapterId> --write  # write derived bboxes into the *fig.json fragments
 *

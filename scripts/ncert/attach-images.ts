 ...
 * questions.image_url. The NCERT PDFs carry a full-page BACKGROUND RASTER (they
 * are scans with an OCR text layer), so there is no extractable image object to
 * pull — the figure has to be cropped as a bbox region off the rendered page,
 * exactly as for the NEET booklets. Takes FRACTIONAL bboxes (0-1 of the page,
 * display-independent) so a vision agent can give them off the rendered PNGs.
 *
 * NCERT's per-question figure load is TINY and that was measured, not assumed:
 * scoping a `Fig N.M` scan to the text BETWEEN an EXERCISE header and the next
 * structural heading finds 2 in the whole of Class 12 Maths, both in Vector
 * Algebra (Fig 10.6 in Ex 10.1, Fig 10.18 in Ex 10.2). A whole-chapter count
 * says 23 for that chapter and is the wrong number — nearly every `Fig` ref sits
 * in the teaching prose, where it is expository and needs no crop.
 *
 *   npx tsx scripts/ncert/attach-images.ts <chapterId>          # dry-run: crop to out/ for eyeballing
 *   npx tsx scripts/ncert/attach-images.ts <chapterId> --apply  # upload + set image_url
 *

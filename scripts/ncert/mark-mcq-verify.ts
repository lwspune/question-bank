 ...
 *
 *   npx tsx scripts/ncert/mark-mcq-verify.ts <chapterId>          # report
 *   npx tsx scripts/ncert/mark-mcq-verify.ts <chapterId> --write  # patch the file
 *
 ...
 * or the check stops being independent. The diff happens HERE: read each row's
 * committed correct option, compare to the agent's derived_answer, stamp
 * `matches_current`. apply-solutions.ts then flags every false LOUD for a manual
 * re-key; it never auto-re-keys, which needs a human adjudication against the
 * printed option TEXT (see the option-ordering trap in CROSSCHECK_BRIEF.md).
 */
...
async function main() {
  ...
  if (write) {
    ...
    // adjudication, not a verdict (the grounding HELD-queue rule).
    const result = await recordMcqVerifyReviews(db, { pipeline: "ncert", artifactId: id, rows: verified });
  ...
  console.log(
    `\n${mismatches} mismatch(es), ${nulls} null(s).` +
      (mismatches || nulls
        ? " Adjudicate each against the printed option TEXT BEFORE flipping PUBLIC."
        : " Independent derivation agrees with every committed key.")
  );

/**
 * Commit one NDA Mathematics paper into the bank — PRIVATE, question_kind='pyq'
 * — via commitStaged (dedup / taxonomy / content_hash) plus an upload_jobs row.
 *
 *   npx tsx scripts/nda-pyq/commit.ts <paperId>          # dry-run (validate only)
 *   npx tsx scripts/nda-pyq/commit.ts <paperId> --apply  # write
 *
 * Reads data/<paperId>.questions.json (from merge.ts) + data/<paperId>.answers.json
 * (the blind derivation pass — see README.md).
 *
 * ALWAYS PRIVATE, AND THAT IS LOAD-BEARING HERE. No UPSC booklet prints a key,
 * so every answer in this corpus is DERIVED. A single blind pass measures ~95%
 * on UPSC papers in this repo — about 6 wrong in 120 — and these rows feed the
 * /mock runner, which grades real students against a FROZEN score. So publishing
 * waits for the external key to be reconciled (reconcile-key.ts) and is a
 * separate, deliberate act, never a side effect of committing.
 *
 ...
 * and on the sibling CDS Mathematics corpus 4 of 5 key disagreements were the
 * KEY's error, not ours.
 */

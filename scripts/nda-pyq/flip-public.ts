/**
 * Publish a committed NDA Paper I (Mathematics) paper — PRIVATE -> PUBLIC.
 *
 *   npx tsx scripts/nda-pyq/flip-public.ts <paperId>            # dry-run: gates only
 *   npx tsx scripts/nda-pyq/flip-public.ts <paperId> --apply    # publish
 *   npx tsx scripts/nda-pyq/flip-public.ts <paperId> --revert --apply   # back to PRIVATE
 *
 * Scoped to ONE paper's `source_file` so it can never reach another sitting, or
 * the NDA English / General-Ability rows that share this exam.
 *
...
async function main() {
  ...
  console.log(`\nreversible: npx tsx scripts/nda-pyq/flip-public.ts ${paper.id} --revert --apply`);
}

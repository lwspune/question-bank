 ...
 *
 *   npx tsx scripts/nda-pyq/audit-solutions.ts            # whole subject
 *   npx tsx scripts/nda-pyq/audit-solutions.ts 2023-1     # one paper
 *   npx tsx scripts/nda-pyq/audit-solutions.ts --json     # machine-readable work list
 *
...
import { EXAM_ID, PAPERS, SUBJECT_ID } from "./config";
import { auditSolution, type SolutionFinding } from "../cds-maths/solutionText";

/** Mathematics only. The NDA exam also holds English and eight General-Ability
 *  subjects, which are different corpora with their own ingestion history. */
...
async function main() {
  ...
  // PAPERS is an ARRAY here (cds-maths keys it by id), so look it up rather
  // than indexing — an index would silently yield undefined and audit the
  // whole subject instead of the one paper asked for.
  const sourceFile = paperId ? PAPERS.find((p) => p.id === paperId)?.sourceFile : undefined;

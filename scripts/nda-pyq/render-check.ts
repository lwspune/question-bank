 ...
 *
 *   npx tsx scripts/nda-pyq/render-check.ts <paperId>
 *   npx tsx scripts/nda-pyq/render-check.ts <paperId> --self-test
 *
 ...
 *
 * This script main() is a near-copy of scripts/cds-maths/render-check.ts. The
 * logic that matters is already shared (parseLatex + the app's own KaTeX); what
 * is duplicated is argument handling and file paths, which cannot be shared
 * without a config-injection layer that would be longer than the duplication.
 */

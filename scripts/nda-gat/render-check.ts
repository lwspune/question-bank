/**
 * Render every math zone of a merged GAT paper through the repo's OWN KaTeX, and
 * report the ones KaTeX refuses.
 *
 *   npx tsx scripts/nda-gat/render-check.ts <paperId>
 *   npx tsx scripts/nda-gat/render-check.ts <paperId> --self-test
 *
 ...
 *
 * ON A GAT PAPER THE BIGGEST CONSUMER OF THIS PROBE IS NOT MATHS. Roughly half
 * the English questions carry `\(\underline{\text{word}}\)`, which is a math
 * zone like any other and which the paper is unanswerable without — so a
 * malformed underline is both a render failure AND a lost question. Physics and
 * Chemistry supply the rest.
 *
 ...
 *     count printed below is the guard: zero zones on this paper means the probe
...
import { normalizeQuestions, ENGLISH_SUBJECT, type GatTQ } from "./lib";
...
function selfTest() {
  const mustFail = [
    ...
    ["\\underline{\\text{both}", "unclosed underline — the Part A failure mode"],
  ];
  let bad = 0;
  for (const [tex, why] of mustFail) {
    const err = tryRender(tex);
    console.log(`  ${err ? "rejected" : "ACCEPTED"}  ${JSON.stringify(tex).padEnd(28)} (${why})`);
    if (!err) bad += 1;
  }
  // The forms this corpus actually uses. A probe that rejects legitimate input
  // is as useless as one that accepts bad input, and noisier.
  const good = [
    "\\underline{\\text{both}}",
    "\\underline{\\textit{Jataka}}",
    "\\frac{1}{4\\pi\\varepsilon_0}\\left(\\frac{2}{9}\\right)",
    "\\text{NC}^{-1}",
    "5386\\ \\text{Å}",
    "\\text{FeSO}_4(\\text{aq})",
    "10\\ \\text{m/s}^2",
...
function main() {
  ...
  // SOLUTIONS ARE IN SCOPE. A solution reaches the student through the same
  // KaTeX path as a stem, so unconvertible markup there fails exactly as
  // visibly — and solutions are the half most likely to carry hand-authored
  // maths, because stems are transcribed from a printed page while solutions are
  ...
  for (const q of questions) {
    ...
    for (const [name, val] of fields) {
      ...
      for (const tex of zonesOf(val)) {
        zones += 1;
        if (q.subject === ENGLISH_SUBJECT) englishZones += 1;
        if (tex.includes("\\underline")) underlines += 1;
        const err = tryRender(tex);
  ...
  console.log(
    `${paper.id}: ${questions.length} questions, ${zones} math zones rendered ` +
      `(${englishZones} in English, ${underlines} carrying \\underline)`
  );
  if (!zones) {
    // The Chemistry false-green, made loud instead of silent.
    console.log(
      `\nZERO math zones on a paper whose English half is built on \\underline and\n` +
        `whose Physics and Chemistry questions carry notation — this probe is broken,\n` +
        `not the data. Check that parseLatex still emits "inline" | "block".`
    );
    process.exit(1);
  }
  if (failures.length) {

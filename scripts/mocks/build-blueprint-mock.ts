...
import { parseBlueprint, assertBlueprint, type AllocRow, type Difficulty } from "./blueprintSpec";
...
const FILLS: Fill[] = [
  {
    chapter: "Probability", subtopic: "Bounds on Probability", difficulty: "HARD",
    ids: ["b1969784-20e0-4652-9d57-ce2e85849e8d"], source: "Worksheets",
  ...
  {
    chapter: "Statistics", subtopic: "Regression and Correlation", difficulty: "HARD",
    ids: ["02cd6233-5b23-4d1f-bc81-2f1119de8666"], source: "Worksheets",
  ...
  {
    chapter: "3D Geometry", subtopic: "Distance, Section, and Collinearity in 3D", difficulty: "HARD",
    ids: ["00149470-ea98-4f37-8301-723ccc4442dc"], source: "Worksheets",
  ...
  {
    chapter: "Properties of Triangle", subtopic: "Sine and Cosine Rules — Solving Triangles", difficulty: "HARD",
    ids: [
  ...
  {
    chapter: "Binomial Theorem", subtopic: "Integer and Fractional Parts of Binomial Expressions", difficulty: "HARD",
    ids: [
 ...
 */
const SLOT_LENT: { chapter: string; subtopic: string; difficulty: Difficulty; to: string }[] = [
  {
 ...
 */
type Pick = AllocRow & { id: string; actual: Difficulty; family: Family; text: string; fill?: Fill };
...
function structurallyClean(r: Row): boolean {
  ...
  return correct === 1 && distinct === r.options.length && r.options.length === 4;
}
...
async function fetchByIds(client: SupabaseClient, ids: string[]): Promise<Map<string, Row>> {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    for (const q of (data ?? []) as any[]) {
      out.set(q.id, {
        id: q.id, difficulty: q.difficulty, chapter: q.chapters?.name ?? '',
        subtopic: q.subtopics?.name ?? null, source_file: q.source_file,
        set_id: q.set_id, context: q.context, text: q.text ?? '', options: q.options ?? [],
      });
    }
  }
  return out;
...
function orderByRule3(picks: Pick[]): Pick[] {
  ...
  for (const p of [...picks].sort((a, b) => chapterFrac.get(a.id)! - chapterFrac.get(b.id)!)) {
...
async function main() {
  ...
  const [pool, used] = await Promise.all([fetchPool(client), fetchUsed(client)]);

  const dropped = { used: 0, structural: 0, setBound: 0, dashedTable: 0 };
  const eligible = pool.filter((r) => {
    if (used.has(r.id)) { dropped.used++; return false; }
    if (!structurallyClean(r)) { dropped.structural++; return false; }
    if (r.set_id !== null || r.context !== null) { dropped.setBound++; return false; }
    if (dashedTable((r.context ?? "") + " " + r.text)) { dropped.dashedTable++; return false; }
    return true;
  });
  console.log(
...
` +
    ...
    for (const c of unresolved) {
      const near = [...new Set(pool.filter((r) => r.chapter === c.chapter).map((r) => r.subtopic))]
        .filter(Boolean)
        .filter((n) => {
  ...
  if (unclean.length) {
    console.error('DECLARED FILLS FAIL THE STRUCTURAL CHECK: ' + unclean.map((r) => r.id).join(', '));
    process.exit(1);
  }
  const alreadyUsed = [...fillRows.values()].filter((r) => used.has(r.id));
  if (alreadyUsed.length) {
    console.error('DECLARED FILLS ALREADY IN A PAPER: ' + alreadyUsed.map((r) => r.id).join(', '));
    process.exit(1);
  }
  ...
  const cellKey = (c: { chapter: string; subtopic: string; difficulty: Difficulty }) =>
  ...
  for (const cell of alloc) {
    ...
    if (fill) {
      for (const id of fill.ids) {
        const r = byId.get(id);
        if (!r) { shortfalls.push({ ...cell, got: 0 }); break; }
        taken.add(id);
        picks.push({ ...cell, id, actual: r.difficulty, family: "declared fill", text: r.text, fill });
      }
      continue;
    }
    ...
    for (const r of got) {
      taken.add(r.id);
      picks.push({ ...cell, id: r.id, actual: r.difficulty, family: familyOf(r.source_file), text: r.text });
    }
  ...
  console.log(`  difficulty  E${byDiff("EASY")} / M${byDiff("MODERATE")} / H${byDiff("HARD")}   (target E30 / M58 / H32)`);
  console.log(
  ...
  const famTotal = byFam("booklet") + byFam("mock paper") + byFam("declared fill");
  if (famTotal !== picks.length)
    console.log(`  !! source counts sum to ${famTotal}, not ${picks.length}`);
  for (const f of FILLS)
    console.log(`    fill  ${f.chapter} / ${f.subtopic} / ${f.difficulty}  <- ${f.source} (${f.ids.length})`);
  console.log("");

  const perChapter = new Map<string, number>();
  for (const p of picks) perChapter.set(p.chapter, (perChapter.get(p.chapter) ?? 0) + 1);
  ...
  if (shortfalls.length) {
    console.log(`UNFILLED CELLS (${shortfalls.length}) — no eligible question exists:`);
    for (const s of shortfalls)
      console.log(`  ${s.chapter} / ${s.subtopic} / ${s.difficulty}: got ${s.got} of ${s.n}`);
    console.log("");
  }
  ...
  const quart: Record<Difficulty, number[]> = {
    EASY: [0, 0, 0, 0], MODERATE: [0, 0, 0, 0], HARD: [0, 0, 0, 0],
  };
  ordered.forEach((p, i) => quart[p.actual][q(i)]++);
  let adjacent = 0;
  for (let i = 1; i < ordered.length; i++) if (ordered[i].chapter === ordered[i - 1].chapter) adjacent++;
  ...
  if (picks.length !== demand) {
    console.error(`REFUSE: ${picks.length} of ${demand} selected — fill the gaps before writing.`);
    process.exit(1);
  }
  ...
  const paperId =
    ...
    (await createPaper(client, {
      orgId: ORG_ID, createdBy: CREATED_BY, title: TITLE, examId: EXAM_ID,
      template: [{ key: "mathematics", label: "Mathematics", targetCount: demand, assignedTo: [] }],
    }));
  console.log(`paper ${paperId}${(existing as any)?.id ? " (existing)" : " (created)"}`);

  let added = 0;
  for (const p of ordered) {
    await addQuestion(client, paperId, p.id, { sectionKey: "mathematics", addedBy: CREATED_BY });
    added++;

...
**Four things the solver prompt MUST ask for, each earned from a miss:**

1. **The VALUE, not just the letter.** What gets compared is the quantity derived, so a
   letter with no quantity behind it cannot be adjudicated.
2. **Check every PAIR of options for equivalence before naming one.** Two options can be
   written differently and denote the same set or number — `(A u B) - (A n B)` and
   `(A' u B') - (A' n B')` are the same set under De Morgan. Four distinct strings can
   still be two correct answers, and every structural gate passes such a row.
3. **Mind the TYPE of the object an option denotes.** A scalar and a matrix are not equal
   even when their numbers coincide: `det(adj P) = 64` is answered by none of `P^27`,
   `P^17`, `P^6`, because `P^6 = 64*I` is a matrix. A solver that ignores type will name
   the suggestive option.
4. **Say whether an option is the COMPLETE answer or only a PROPER SUBSET of it** — for a
   "solutions of the equation" question the difference decides whether the paper is
   shippable, and it is invisible to any check that only compares one representative value.

**And read a LOW-CONFIDENCE flag as a request for a second pass, never as licence to
override.** On Mock 4 a solver returned the wrong letter at low confidence while stating in
its own note that the strict reading forced the keyed one; a second pass told to watch the
type agreed with the key, and the printed page and the booklet's own key then agreed too.
...
| 2026-08-31 | Built **Mock 4** (paper `0232fca2`): 120 q, **30 E / 59 M / 31 H**, all 30 chapter names, 0 repeats against the three earlier mocks. Source mix as BUILT: **NDA 118 / JEE Mains 1 / Worksheets 1**, against Mock 3's NDA 112 / Worksheets 7 / JEE 1 — the PYQ-fill finding below plus four NDA-PYQ replacements took the paper to 98% NDA content. Nine cells came up short — ten questions, and the shortfall was exactly 1 EASY / 2 MODERATE / 7 HARD, so filling it lands precisely on the 30/58/32 target. The 59/31 as BUILT is the JEE fill's own-bank MODERATE label again (RULE 2a), reported not designed. |
| 2026-08-31 | **THE FILL SOURCE CHANGES, and it is a measurement rather than a preference: for SEVEN of the nine short cells an unused NDA PYQ exists at the EXACT subtopic AND the EXACT difficulty.** Mocks 2 and 3 filled those same cells from Worksheets - not because a PYQ was absent, but because the builder's pool is `question_kind = 'practice'` and nothing had ever looked at the PYQ side. An NDA PYQ is the better fill twice over: same bank, so RULE 2a's difficulty translation does not apply at all, and it is a real question from the exam being mocked. Only Binary Arithmetic MODERATE and Binomial 'Integer and Fractional Parts' HARD still need a cross-bank fill, and both are genuinely EMPTY on the NDA side rather than merely depleted. **Check the PYQ side before reaching across banks.** |
| 2026-08-31 | **TWO CORRUPT STEMS CAUGHT BEFORE THE PAPER WAS BUILT — the first time that has happened.** Mocks 1-3 found every one at review time; here RULE 2a's requirement to READ a fill rather than trust its label is what exposed them, because the arithmetic did not close. **Q25** (NDA Sep 2024) stored 'sqrt(2)a - b' where the paper prints '2a - b'; as stored NO option was reachable, and the printed form gives exactly the stored key. **Q426** (Sequence and Series booklet) had its stem AND all four options corrupt and carried NO SOLUTION AT ALL, which is why nothing had ever caught it - as stored the A.P. condition had discriminant -140, so the question had no real answer. Both source-verified; both keys were already right. |
| 2026-08-31 | **TWO GENUINE WRONG KEYS, and they fail in opposite directions.** **Q69** (NDA 2021-I) is the rarer shape: the stem is FAITHFUL to the printed page, so it is the ANSWER that is wrong. The cross product's three components vanish at k=23, k=5+/-sqrt(2) and k=23/2 with no common root, so the points are never collinear - Zero, not One. Correcting it is legitimate because the paper publishes no key and the stored value came from a prep-house compilation, so this fixes a derivation rather than overriding UPSC. **Q139** (Complex Numbers booklet) is the inverse: OUR transcription flipped a sign in option D and thereby turned a WRONG option into a correct one, giving the row two correct answers. The booklet prints '-3, 1-2w, -1-2w^2', a mixed list that is not the root set; restoring it leaves one correct option. The booklet's own key also points at that option, so the book is wrong too. |
| 2026-08-31 | **SWAPPING A QUESTION OUT OF A PAPER MAKES IT ELIGIBLE AGAIN, and Mock 4 selected THREE Mock-3 rejects straight back.** The builder's only durable exclusion was 'is it in some paper', and Mock 3's rejections lived in a script comment no query can see - so the ambiguous AM/median/GM question and the unprintable truncated-stem set question both returned, and a third of the same kind besides. **Fixed at the cause:** the builder now also excludes any question whose LATEST `question_reviews` verdict is `unverifiable`, so the exclusion is derived from the audit trail rather than kept by hand, and repairing a question returns it to the pool by itself. All three are recorded. Worth knowing the render gate PASSES the truncated one, because an UNPAIRED '$' renders as a literal dollar sign rather than raising a KaTeX error. |
| 2026-08-31 | **RULE 5: four solutions rewritten, and one of them derived the WRONG OPTION.** The matrix-power-sum question summed n terms where the series has n-1, reaching n[[1,(n-1)/2],[0,1]] - which is a printed option, just not the keyed one, so a wrong term count landed on a wrong answer rather than on nothing. **The letter-mismatch probe could not see it: that solution ends in a matrix, not an option letter.** The others were an assertion-only 'Ans : (c)** Both I and II are true' on a row where both statements need real arguments, a solution that listed six pairs and then trailed off before stating the count, and a Hindi-and-artifact remnant from the .docx ingest. After the rewrites `triage-dump` reports **0 flagged**. |
| 2026-08-31 | **Both repair tools became RE-RUNNABLE, which they were not.** An `--apply` makes every one of its own `expect` strings stale, so adding a fix found later in the same review aborted the whole batch on the fixes that had already landed - and the swap tool refused identically once its outgoing question was gone. Now an ALREADY-APPLIED edit is a SKIP while any other unexpected state still refuses, in both. `swap-paper-question.ts` also takes the paper id on the command line and keys its swaps BY PAPER (the `FILL_SETS` shape), so an applied swap stays readable as the record of a decision instead of being overwritten by the next paper's. |
| 2026-08-31 | **Q659 is the case where the BLIND PASS was wrong and the key was right, and the reason is worth keeping: an option's TYPE.** det(adj P) for P = 2I(3) is 64, a SCALAR, while the printed options are P^27, P^17, P^6 — MATRIX powers, and P^6 = 64*I(3) is not the number 64. The first solver read P^6 as the scalar 2^6 and returned that option at LOW confidence, saying in its own note that a strict literal reading would force 'None of these'. A second pass told to watch the type of the object returned the keyed option. Then all three independent grounds agreed: our transcription is faithful to the printed page, the mathematics gives the scalar, and the BOOKLET'S OWN KEY says the same. **Added to the blind-solver prompt: a quantity and a matrix are not equal even when their numbers coincide, and a low-confidence flag on a letter is worth a second pass rather than an override.** |
| 2026-08-31 | **THREE questions in this paper had NO CORRECT ANSWER AT ALL, all three source-verified as faithful transcriptions of a defective printed question, and all three replaced.** The lim(x->0) 1/(3 - 2^(1/x)) whose two-sided limit does not exist while both one-sided values are printed as options; the round-table question whose 'five chairs of DIFFERENT COLOURS' makes the count 5! = 120, absent from its own options, keyed instead at 4! = 24; and the truncated-stem set question. **The test applied was HARM, not shape** (the Mock 3 duplicate-option precedent): replace when a student reasoning correctly finds no option and must guess. By the same test Q1227 was KEPT — its keyed option is an incomplete solution set, omitting 3pi/4 and 5pi/4, but the other three options contain no solution at all, so a correct reasoner still lands on it. The incompleteness is named in the solution rather than hidden. |
| 2026-09-02 | Built **Mock 5** (paper `f918efd4`): 120 q, **30 E / 59 M / 31 H**, all 30 chapter names, 0 repeats. Source mix as BUILT: **NDA 116 / Worksheets 3 / JEE Mains 1**. Eleven cells short and TWELVE questions — the deepest shortfall yet — of which EIGHT came from NDA PYQs at the exact subtopic and difficulty, applying the Mock 4 finding from the start. |
| 2026-09-02 | **THE BOARD-BANK SWEEP, run because the pool is thinning and it settles which corpora can feed a printed mock at all.** Free 4-option Maths MCQs, and the share carrying NO worked solution: **CBSE Class 12 = 756 at 0% unsolved · MH HSC Class 12 = 268 at 0% · MH State Board Class 11 = 203 at 65% · MH State Board Class 9 = 83 at 71% · CBSE Class 11 = ZERO MCQs** (that book contains none). MH Class 11 genuinely carries ~10 MCQs per chapter, but two thirds have no solution — INCLUDING ALL THREE of its free HARD rows — and a printed mock needs an answer key with working, so they are unusable as they stand. CBSE 12 and MH HSC 12 are fully solved and usable, but neither book teaches Binomial Theorem, Binary Numbers or Properties of Triangle, which were exactly the cells still open. **Logarithms and Binary Numbers are NDA-ONLY chapters — no board syllabus has them, so no board bank can ever fill those two cells.** |
| 2026-09-02 | **Reviewed Mock 5 (RULE 4 + RULE 5): 120/120 agree, and the pattern INVERTS Mock 3's.** There every letter-level disagreement was a corrupt transcription under a correct key; here most transcriptions are FAITHFUL and the defects belong to the printed books. **THREE key errors were confirmed against the page AND against the booklets' own printed answer tables** — and in two of the three the book prints the fuller correct option and keys the narrower one. **Q1195**: tan x - sin x = 1 - tan x sin x factors as (1+sin x)(tan x - 1) = 0, and the sin x = -1 branch forces cos x = 0 where tan x is UNDEFINED, so the keyed option asserts extraneous roots of the CLEARED equation; the book prints the correct set as its option (c). **Q1975**: cos^2(gamma) = 1/4 admits both 60 and 120 degrees — two genuinely different lines — and the book keys 60 alone while printing 'both' as an option. **Q2507**: OUR option texts A and B are swapped against the page and the key letter was carried across without accounting for it, which is why the stored solution derives the right points and then writes 'Hence (A)'. |
| 2026-09-02 | **FOUR of the flagged rows needed NO change once the page was read, which is the argument for reading it first.** Q1052's value is identically 8cos(2 theta), which matches no substantive option — and 'None of these' IS the printed key, so a NO_OPTION flag was the right finding and the wrong conclusion. Q2973's stem looked like a corrupted Clairaut equation (which would move the answer from B to D), and the booklet really does print dp/dx. Q2179's odd domain [0, inf) is the book's own. Q1052 and Q2179 kept their keys untouched; Q2973 needed only a distractor restored ('3, 1' where the page prints '2, 1'). **Solve as printed and report the discrepancy — do not 'correct' a stem toward the form you recognise.** |
| 2026-09-02 | **A NEW DEFECT CLASS, and RULE 1 is one-directional.** A row whose stem says 'in respect of the ABOVE frequency distribution' had `set_id` and `context` BOTH null — the table was simply gone, so nothing in it could be evaluated. RULE 1 excludes rows that HAVE a context, on the grounds that a set is all-or-nothing; it has no guard against a row that NEEDS one and lost it. **Measured before generalising: exactly TWO PUBLIC NDA Maths rows reference absent DATA this way, so the class is rare rather than systemic.** A first probe counting every 'the above' matched 145 rows and was measuring the wrong thing — nearly all say 'which of the above statements', where the statements are listed in the stem itself. Replaced by a PYQ that carries its own frequency table. |
| 2026-09-02 | **A CONTAMINATION CHANNEL IN THE BLIND PASS, disclosed by a solver rather than caught by tooling: `CLAUDE.md` is auto-loaded into every subagent's context, and its Decisions log names answer letters.** One solver reported seeing a Properties-of-Triangle passage stating that a substitution 'makes the keyed 30 degrees correct' — for a row in its own batch. It stopped on sight, said so, and its derivation was independently forced anyway (A+B=120 gives C=60; sin A = 1/2; the 150-degree root needs B = -30). So the blind pass is blind to the KEY IN THE DUMP but NOT to the repository's own documentation, and for any question whose adjudication has been written up it is weakened. The control that worked is the instruction to DISCLOSE; it is now in the solver prompt, and later batches were asked to report explicitly whether CLAUDE.md named an answer for their rows (batch 9: it did not). |
| 2026-09-02 | **RULE 5: four solutions rewritten, one of which argued a student into the wrong letter.** A complex-number row derived Arg = -pi/2 correctly and then wrote 'therefore z-bar*omega = -1' — unit modulus at -pi/2 is -i, and -1 is itself a printed option, so the working contradicted the key it was attached to. Also rewritten: a median row where the cumulative frequency lands EXACTLY on N/2, so the raw-data rule gives 4.5 which is not offered and the keyed 5 depends on the (N+1)/2 discrete-series convention — the tie IS the question, and the old solution asserted a bare rule without saying so. Two remaining triage flags are the known false-positive classes (a set NAMED A read as option A; a stray numeral read as an option value) and both keys were confirmed blind. |

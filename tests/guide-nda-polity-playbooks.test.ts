/**
 * Integration test for the /guide/nda-polity playbook tree.
 *
 * Mirrors the equivalent english + physics + chemistry + biology + geography
 * + history tests — locks the invariants the guide can't ship without:
 ...
 *      NDA Polity taxonomy via resolveTaxonomy — catches the silent
...
import { PLAYBOOK_DETAILS } from "@/app/guide/nda-polity/_data/playbook-details";
...
describe("nda-polity playbooks — static structure", () => {
...
describe.skipIf(!HAS_ENV)(
  "nda-polity playbooks — live taxonomy resolution",
  () => {
    ...
    it("every playbook's chapter + subtopic names resolve under live NDA Polity taxonomy", async () => {
      const taxonomy = await resolveTaxonomy(client, "NDA", "Polity");

...
import { deriveStudents, providerLabel, type AuthUserLite } from "@/lib/students/derive";

const u = (id: string, created: string, provider?: string, email?: string): AuthUserLite => ({
  id,
...
describe("deriveStudents", () => {
  ...
  it("derives email + provider label", () => {
    const rows = deriveStudents([u("s1", "2026-01-01T00:00:00Z", "google", "x@y.com")], new Set());
    expect(rows[0]).toMatchObject({ email: "x@y.com", provider: "Google" });
  });

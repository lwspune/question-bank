/**
 * Integration test for the /guide/nda-biology playbook tree.
 *
 * Mirrors the equivalent english + physics + chemistry tests — locks the
 * invariants the guide can't ship without:
 ...
 *      NDA Biology taxonomy via resolveTaxonomy — catches the silent
...
import { PLAYBOOK_DETAILS } from "@/app/guide/nda-biology/_data/playbook-details";
...
describe("nda-biology playbooks — static structure", () => {
...
describe.skipIf(!HAS_ENV)(
  "nda-biology playbooks — live taxonomy resolution",
  () => {
    ...
    it("every playbook's chapter + subtopic names resolve under live NDA Biology taxonomy", async () => {
      const taxonomy = await resolveTaxonomy(client, "NDA", "Biology");

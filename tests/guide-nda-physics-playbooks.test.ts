/**
 * Integration test for the /guide/nda-physics playbook tree.
 *
 * Mirrors the equivalent english test (tests/guide-nda-english-playbooks.test.ts)
 * — locks the invariants the guide can't ship without:
 ...
 *      NDA Physics taxonomy via resolveTaxonomy — catches the silent
 *      broken-CTA failure mode after a taxonomy rename.
 *
 * Skips the live integration block if env vars aren't loaded (mirrors the
 * convention from filter-facets.test.ts and the english test).
 */
...
import {
  PLAYBOOKS,
  PLAYBOOK_SLUGS,
} from "@/app/guide/nda-physics/_data/playbooks";
import { PLAYBOOK_DETAILS } from "@/app/guide/nda-physics/_data/playbook-details";
...
describe("nda-physics playbooks — static structure", () => {
...
describe.skipIf(!HAS_ENV)(
  "nda-physics playbooks — live taxonomy resolution",
  () => {
    ...
    it("every playbook's chapter + subtopic names resolve under live NDA Physics taxonomy", async () => {

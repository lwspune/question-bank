/**
 * Integration test for the /guide/nda-history playbook tree.
 *
 * Mirrors the equivalent english + physics + chemistry + biology + geography
 * tests — locks the invariants the guide can't ship without:
 ...
 *      NDA History taxonomy via resolveTaxonomy — catches the silent
...
import { PLAYBOOK_DETAILS } from "@/app/guide/nda-history/_data/playbook-details";
...
describe("nda-history playbooks — static structure", () => {
...
describe.skipIf(!HAS_ENV)(
  "nda-history playbooks — live taxonomy resolution",
  () => {
    ...
    it("every playbook's chapter + subtopic names resolve under live NDA History taxonomy", async () => {
      const taxonomy = await resolveTaxonomy(client, "NDA", "History");

...
const PAGE_TITLE = "NDA Physics — Teaching Notes";
const PAGE_INTRO =
  "Per-subtopic teaching notes for NDA Physics — built for digital-board lectures and " +
  "student self-study side-by-side. Each chapter breaks down into concept-by-concept units " +
  "with intuition, formula or reference table, authored worked example, a featured PYQ, traps, " +
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "/notes/nda-physics" },
};
...
const CHAPTERS: ChapterCard[] = getNotesChaptersForSubject("nda-physics").map(
  (c) => ({
    slug: c.chapterSlug,
...
const sideNav = [
  { href: "/notes/nda-physics", label: "Chapter index" },
  ...CHAPTERS.map((c) => ({
    href: `/notes/nda-physics/${c.slug}`,
    label: c.chapterName,
  })),
];

export default async function NdaPhysicsNotesIndex() {
  const supabase = createSupabaseAnonClient();
  const taxonomy = await getNotesTaxonomy(supabase, "NDA", "Physics");
  ...
  return (
    <GuideShell
      guideTitle="NDA Physics Notes"
      sideNav={sideNav}
      breadcrumbs={[{ label: "Notes" }]}
    >
      <GuideJsonLd
        type="CollectionPage"
        path="/notes/nda-physics"
        headline={PAGE_TITLE}
        description={PAGE_INTRO}
      />

      <GuideHero
        eyebrow="NDA Physics · Teaching notes"
        title={PAGE_TITLE}
        subtitle={PAGE_INTRO}
      />

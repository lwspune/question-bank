...
const PAGE_TITLE = "MHT-CET Maths — Teaching Notes";
const PAGE_INTRO =
  "Per-subtopic teaching notes for MHT-CET Mathematics — built for digital-board lectures and " +
  "student self-study side-by-side. Each chapter breaks down into concept-by-concept units " +
  "with intuition, formula, an authored worked example, a self-check, Level 1 practice, a featured PYQ, traps, " +
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "/notes/mht-cet-maths" },
};
...
const CHAPTERS: ChapterCard[] = getNotesChaptersForSubject("mht-cet-maths").map(
  (c) => ({
    slug: c.chapterSlug,
...
const sideNav = [
  { href: "/notes/mht-cet-maths", label: "Chapter index" },
  ...CHAPTERS.map((c) => ({
    href: `/notes/mht-cet-maths/${c.slug}`,
    label: c.chapterName,
  })),
];

export default async function MhtCetMathsNotesIndex() {
  const supabase = createSupabaseAnonClient();
  const taxonomy = await getNotesTaxonomy(supabase, "MHT-CET", "Maths");
  ...
  return (
    <GuideShell
      guideTitle="MHT-CET Maths Notes"
      sideNav={sideNav}
      breadcrumbs={[{ label: "Notes" }]}
    >
      <GuideJsonLd
        type="CollectionPage"
        path="/notes/mht-cet-maths"
        headline={PAGE_TITLE}
        description={PAGE_INTRO}
      />

      <GuideHero
        eyebrow="MHT-CET Maths · Teaching notes"
        title={PAGE_TITLE}
        subtitle={PAGE_INTRO}
      />

  ...
  concepts: [
    // C1 — equation, centre, radius
    {
      ...
      authoredExample: {
        ...
        steps: [
          "Match \\(2u = -6,\\ 2v = 8,\\ 2w = -10\\): so \\(u = -3,\\ v = 4,\\ w = -5\\), and \\(d = 1\\).",
          "Centre \\(= (3, -4, 5)\\).",
          "Radius \\(= \\sqrt{(-3)^2 + 4^2 + (-5)^2 - 1} = \\sqrt{9+16+25-1} = \\sqrt{49} = 7\\).",
        ],
        answer: "Radius \\(= 7\\) (centre \\((3,-4,5)\\)).",
      },
      selfCheckExample: {
    ...
    // C2 — diameter form
    {
      ...
      authoredExample: {
        prompt:
          "\\((1,-1,2)\\) and \\((2,1,-1)\\) are the endpoints of a diameter of \\(x^2+y^2+z^2+2ux+2vy+2wz-1=0\\). Find \\(u+v+w\\).",
        steps: [
          "Centre = midpoint of the diameter = \\(\\left(\\tfrac{1+2}{2}, \\tfrac{-1+1}{2}, \\tfrac{2-1}{2}\\right) = \\left(\\tfrac32, 0, \\tfrac12\\right)\\).",
          "Centre \\(= (-u,-v,-w)\\), so \\(u = -\\tfrac32,\\ v = 0,\\ w = -\\tfrac12\\).",
          "\\(u + v + w = -\\tfrac32 + 0 - \\tfrac12 = -2\\).",
        ],
        answer: "\\(u + v + w = -2\\).",
      },
      selfCheckExample: {
    ...
    // C3 — sphere from conditions
    {
      ...
      definition:
        ...
        "origin forces \\(d = 0\\)).",
      authoredExample: {
        prompt: "Find the equation of the sphere with centre \\((-2, 3, 4)\\) and radius 6.",
        steps: [
          "Centre-radius form: \\((x+2)^2 + (y-3)^2 + (z-4)^2 = 36\\).",
          "Expand: \\(x^2 + 4x + 4 + y^2 - 6y + 9 + z^2 - 8z + 16 = 36\\).",
          "Collect: \\(x^2 + y^2 + z^2 + 4x - 6y - 8z - 7 = 0\\).",
        ],
        answer: "\\(x^2 + y^2 + z^2 + 4x - 6y - 8z - 7 = 0\\).",
      },
      selfCheckExample: {
        prompt:
    ...
    // C4 — sphere and plane
    {
      ...
      authoredExample: {
        prompt:
          "Find the diameter of the sphere whose centre is \\((1, -2, 3)\\) and which touches the plane \\(6x - 3y + 2z - 4 = 0\\).",
        steps: [
          "Touches → radius = perpendicular distance from centre to plane.",
          "\\(p = \\dfrac{|6(1) - 3(-2) + 2(3) - 4|}{\\sqrt{36+9+4}} = \\dfrac{|6+6+6-4|}{\\sqrt{49}} = \\dfrac{14}{7} = 2\\).",
          "So radius \\(r = 2\\); diameter \\(= 4\\).",
        ],
        answer: "Diameter \\(= 4\\).",
      },
      selfCheckExample: {
    ...
    // C5 — sphere and axes
    {
      ...
      definition:
        "A sphere of radius \\(r\\) touches all three axes when its centre is equidistant (distance " +
        "\\(\\tfrac{r}{\\sqrt2}\\)... by axis-distance) — concretely the centre coordinates satisfy " +
        "\\(|x_c|=|y_c|=|z_c|\\). Each independent sign choice gives a distinct sphere, so there are " +
        "several such spheres (one per relevant octant configuration).",
      formula: {
        label: "Distance from a point to the z-axis",
        latex: "\\text{dist to } z\\text{-axis} = \\sqrt{x_c^2 + y_c^2}",
      },
      authoredExample: {
        prompt: "What is the number of spheres of radius \\(r\\) touching all three coordinate axes?",
        steps: [
          "Touching all three axes forces the centre to be \\((\\pm a, \\pm a, \\pm a)\\) for a fixed \\(a\\) determined by \\(r\\).",
          "Each coordinate independently takes sign \\(+\\) or \\(-\\): \\(2 \\times 2 \\times 2 = 8\\) sign patterns.",
          "Each gives a genuinely different sphere of the same radius.",
        ],
        answer: "8 spheres.",
      },
      selfCheckExample: {
    ...
    // C6 — locus problems
    {
      ...
      selfCheckExample: {
        prompt:
          "A variable plane through the fixed point \\((a, b, c)\\) cuts the axes at \\(A, B, C\\). What kind of curve is the locus of the centre of the sphere \\(OABC\\) (O the origin)?",
        steps: [
          "If the plane meets the axes at \\((p,0,0),(0,q,0),(0,0,r)\\), the sphere \\(OABC\\) has centre \\(\\left(\\tfrac{p}{2},\\tfrac{q}{2},\\tfrac{r}{2}\\right)\\).",
          "The plane passes through \\((a,b,c)\\): \\(\\tfrac{a}{p}+\\tfrac{b}{q}+\\tfrac{c}{r} = 1\\).",
          "With centre \\((x,y,z)\\), \\(p = 2x\\) etc.: \\(\\tfrac{a}{2x}+\\tfrac{b}{2y}+\\tfrac{c}{2z} = 1\\).",
          "So \\(\\tfrac{a}{x}+\\tfrac{b}{y}+\\tfrac{c}{z} = 2\\) — the locus.",
        ],
        answer: "The surface \\(\\dfrac{a}{x}+\\dfrac{b}{y}+\\dfrac{c}{z} = 2\\).",
      },
      practiceSet: [

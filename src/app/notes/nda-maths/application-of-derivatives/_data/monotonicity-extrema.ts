  ...
  concepts: [
    {
      ...
      definition:
        ...
        "- **Proving an inequality by monotonicity:** to show \\(g(x)>h(x)\\) for \\(x>0\\), set \\(f=g-h\\); if \\(f(0)=0\\) (or \\(f(0)\\ge0\\)) and \\(f'>0\\) on \\((0,\\infty)\\), then \\(f>0\\) there. So \\(x>\\sin x\\) for \\(x>0\\) (from \\(f=x-\\sin x\\), \\(f'=1-\\cos x\\ge0\\)), \\(e^x>1+x\\), \\(\\ln(1+x)<x\\). To COMPARE two numbers like \\(9\\ln7\\) and \\(7\\ln9\\), find a function of one variable that produces both — \\(\\dfrac{\\ln x}{x}\\) is decreasing for \\(x>e\\), so \\(\\dfrac{\\ln7}{7}>\\dfrac{\\ln9}{9}\\), hence \\(7^9>9^7\\); equivalently \\(\\dfrac{x}{\\ln x}\\) is increasing for \\(x>e\\) with minimum \\(e\\) at \\(x=e\\).\n" +
        "- **Length of a monotone interval of a periodic function:** \\(\\sin kx\\) and \\(\\cos kx\\) have period \\(\\tfrac{2\\pi}{k}\\), so each run of increase or decrease lasts HALF a period, \\(\\tfrac{\\pi}{k}\\). Collapse first: \\(2\\cos^2x-1=\\cos2x\\) decreases on intervals of length \\(\\tfrac\\pi2\\), not \\(\\pi\\); \\(3\\sin x-4\\sin^3x=\\sin3x\\) increases on intervals of length \\(\\tfrac\\pi3\\).",

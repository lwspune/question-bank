...
export function generateStaticParams(): Params[] {
  return VECTORS_SLUGS.map((subtopicSlug) => ({ subtopicSlug }));
}
...
export async function generateMetadata({
  ...
  const note = VECTORS_NOTES[params.subtopicSlug];
  if (!note) return { title: "Note not found" };
  return {
    title: `${note.title} — NDA Maths Vectors notes`,
    description: note.oneLineDefinition,
    alternates: {
      canonical: `/notes/nda-maths/vectors/${params.subtopicSlug}`,
    },
  };
}
...
export default async function SubtopicNotePage({
  ...
  return (
    ...
    >
      <GuideJsonLd
        type="Article"
        path={`/notes/nda-maths/vectors/${params.subtopicSlug}`}
        headline={`${note.title} — NDA Maths Vectors notes`}
        description={note.oneLineDefinition}
      />

 ...
 * prompt always, and hides the numbered solution steps + final answer
 * behind a "Show solution" reveal so students attempt before checking.
 *
 * Distinct from WorkedExampleCard (which renders a bank PYQ with options +
 * reveal logic). KaTeX-aware everywhere.
 */
    ...
    >
      ...
      {!showSolution ? (
        <button
          ...
          aria-expanded={false}
          className="mt-4 inline-flex items-center gap-1.5 rounded-md border bg-background px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted transition-colors"
        >

...
const BUCKET_LABEL = {
  "paired-fact":
    "Paired-fact swap (Recall genre — reformer↔movement, ruler↔dynasty, scholar↔text)",
  chronology:
    "Chronology / Sequence (date-anchored — traveller-by-era, freedom-event ordering, EIC dates)",
  verify:
    "Multi-statement Verify (consider the following — partial-credit, universal-claim, match-list)",
} as const;
...
export default async function Traps() {
  ...
  return (
    ...
    >
      <GuideJsonLd
        type="Article"
        path="/guide/nda-history/traps"
        headline="NDA History Traps — Distractor patterns NDA reuses"
        description="How candidates who know History still lose marks. Reformer-movement swap, ruler-dynasty swap, scholar-text swap, Act-year swap, traveller-by-era out-of-sequence, multi-statement partial-credit."
      />
      <GuideHero
        eyebrow="Traps"
        title="How NDA loses you marks even when you know the history"
        subtitle="History distractors are dominantly PAIRED-FACT SWAPS (king↔dynasty, scholar↔text, viceroy↔era, treaty↔year, reformer↔movement) — same genre as Biology. Plus CHRONOLOGY-SEQUENCE traps (traveller-by-era out-of-order, freedom-event mis-sequencing, EIC date confusion). Plus MULTI-STATEMENT VERIFY (partial-credit, universal-claim, match-list misalignment). Different from Chemistry (compound identity), Physics (formula misapplication), English (near-synonym). Each trap below is illustrated on a real PYQ where one exists."
      >
      ...
      {(["paired-fact", "chronology", "verify"] as const).map((bucket) => {
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      ...
                      {trap.affects.map((slug, i) => (
                        <span key={slug}>
                          <Link
                            href={`/guide/nda-history/playbooks/${slug}`}
      ...
      <section className="mt-14 rounded-lg border-2 border-primary/40 bg-primary/5 p-6">
        ...
        <p className="mt-2 font-serif leading-relaxed text-muted-foreground">
          History&rsquo;s verification habit is different from Physics &mdash; there&rsquo;s
          no unit-check or sign-check. The Recall lever is paired-fact recall
          (reformer ↔ movement; ruler ↔ dynasty; scholar ↔ text). The
          Chronology lever is absolute-date anchoring (anchor 15 cold dates
          and bracket-in). The Verify lever is statement-by-statement T/F.
          Match the tier to the question shape.
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-md border bg-card p-4">
            ...
            <p className="mt-1 font-serif text-xs leading-relaxed text-muted-foreground">
              Ram Mohan Roy → Brahmo Samaj 1828. Dayanand → Arya Samaj 1875.
              Akbar → mansabdari + Din-i-Ilahi. Plassey 1757 → Clive vs Siraj.
              State both halves of the pair before picking.
            </p>
          </div>
          <div className="rounded-md border bg-card p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              25 seconds (Chronology)
            </p>
            <p className="mt-1 text-sm font-semibold tracking-tight">
              Absolute-date bracket
            </p>
            <p className="mt-1 font-serif text-xs leading-relaxed text-muted-foreground">
              Anchor 2–3 absolute dates you&rsquo;re sure of, then BRACKET
              the unknown event in. Bernier 17C Aurangzeb → so must be after
              Monserrate 16C Akbar. Quit India 1942 → so must be after
              Civil Disobedience 1930. Don&rsquo;t pick on &lsquo;recently I
              read this&rsquo; — use dates.
            </p>
          </div>
          <div className="rounded-md border bg-card p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <p className="mt-1 font-serif text-xs leading-relaxed text-muted-foreground">
              ...
              match to the option that lists EXACTLY your T set. Don&rsquo;t
              pick partial recognition — uncertain statements make the whole
              question uncertain. Watch for universal claims (all / every /
              always).
            </p>
          </div>
        </div>
        <p className="mt-4 font-serif text-sm leading-relaxed text-muted-foreground">
          <strong className="font-semibold text-foreground">
            ...
            Recall; the absolute-date anchor on Chronology.
          </strong>{" "}
          Most History distractors fall to one of these. A guess at 5 seconds
          without either is negative-EV; a 10-second check + skip is strictly better.
        </p>
      </section>

      <PrevNextNav
        prev={{ href: "/guide/nda-history/trends", label: "Trends" }}
        next={{ href: "/guide/nda-history", label: "Back to Overview" }}
      />
    </GuideShell>
  );
}

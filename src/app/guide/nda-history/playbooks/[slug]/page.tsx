...
import RelatedHistoryPlaybooks from "@/app/guide/_components/RelatedHistoryPlaybooks";
...
export async function generateMetadata({
  ...
  return {
    title: `${playbook.name} — NDA History playbook`,
    description: playbook.summary,
    alternates: { canonical: `/guide/nda-history/playbooks/${params.slug}` },
  };
}
...
export default async function PlaybookDetail({ params }: { params: Params }) {
  ...
  const [taxonomy, examples] = await Promise.all([
    resolveTaxonomy(supabase, "NDA", "History"),
    detail
  ...
  const prev =
    idx > 0
      ? {
          href: `/guide/nda-history/playbooks/${PLAYBOOKS[idx - 1].slug}`,
          label: PLAYBOOKS[idx - 1].name,
        }
      : { href: "/guide/nda-history/playbooks", label: "All playbooks" };
  const next =
  ...
  return (
    <GuideShell
      ...
      breadcrumbs={[
        ...
        { href: "/guide/nda-history/playbooks", label: "Playbooks" },
        { label: playbook.name },
      ]}
    >
      <GuideJsonLd
        type="Article"
        path={`/guide/nda-history/playbooks/${params.slug}`}
        headline={`${playbook.name} — NDA History playbook`}
        description={playbook.summary}
      />
      <GuideHero
        eyebrow="Playbook"
          ...
          {detail.traps.length > 0 && (
            <section className="mt-12">
              ...
              <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
                ...
                <a className="text-primary hover:underline" href="/guide/nda-history/traps">
                  Traps section
                </a>{" "}
      ...
      {detail && <RelatedHistoryPlaybooks slugs={detail.relatedSlugs} />}

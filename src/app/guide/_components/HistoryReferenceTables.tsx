...
import { REFERENCE_CLUSTERS } from "@/app/guide/nda-history/_data/timeline-and-pairs";

/**
 * Multi-domain reference-tables renderer for /guide/nda-history/timeline-and-pairs.
 *
 * Parallel to BiologyReferenceTables + GeographyReferenceTables — same
 * multi-domain shape (each cluster carries its own column headers since
 * History's named facts span five distinct domains: era timeline /
 * rulers↔dynasty / reformers↔movement / scholars↔texts / Acts↔year).
 * Kept as its own component because the playbook link path is hard-coded
 * per guide (same precedent as RelatedHistoryPlaybooks).
 *
 ...
 * name in amber.
 */
export default function HistoryReferenceTables() {
  return (
                ...
                {cluster.entries.map((e) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right">
                      {e.playbookSlug && (
                        <Link
                          href={`/guide/nda-history/playbooks/${e.playbookSlug}`}

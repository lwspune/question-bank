...
import { REFERENCE_CLUSTERS } from "@/app/guide/nda-geography/_data/reference-tables";

/**
 * Multi-domain reference-tables renderer for /guide/nda-geography/reference-tables.
 *
 * Parallel to BiologyReferenceTables — same multi-domain shape (each cluster
 * carries its own column headers since Geography's named facts span four
 * different domains: rivers/peaks/minerals-crops/winds-zones). Kept as its
 * own component because the playbook link path is hard-coded per guide
 * (same precedent as RelatedGeographyPlaybooks).
 *
 ...
 * name in amber.
 */
export default function GeographyReferenceTables() {
  return (
                ...
                {cluster.entries.map((e) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right">
                      {e.playbookSlug && (
                        <Link
                          href={`/guide/nda-geography/playbooks/${e.playbookSlug}`}

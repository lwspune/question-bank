...
export function generateStaticParams(): Params[] {
  // Every playbook gets a static page — even ones whose detail content is
  // sparse still render the trigger + sub-skills + traps from the entry.
...
export default async function PlaybookDetail({ params }: { params: Params }) {
  ...
  const [taxonomy, examples] = await Promise.all([
    resolveTaxonomy(supabase, "NDA", "English"),
    detail ? loadWorkedExamples(supabase, detail.exampleQuestionIds) : Promise.resolve([]),
  ]);

  // Resolve the playbook's chapter+subtopic names → UUIDs for the BrowseLink CTAs.
  ...
  // through the catalog in bucket order.
  const idx = PLAYBOOKS.findIndex((p) => p.slug === params.slug);
  const prev =
    idx > 0
      ? {
          href: `/guide/nda-english/playbooks/${PLAYBOOKS[idx - 1].slug}`,
          label: PLAYBOOKS[idx - 1].name,
        }
      : { href: "/guide/nda-english/playbooks", label: "All playbooks" };
  const next =
  ...
  const stats = [
    ...
    { value: String(playbook.subtopics.length), label: "subtopic(s) covered" },
  ...
  return (
    <GuideShell
      ...
      breadcrumbs={[
        ...
        { href: "/guide/nda-english/playbooks", label: "Playbooks" },
        { label: playbook.name },
      ]}
    >
      <GuideJsonLd
        type="Article"
        path={`/guide/nda-english/playbooks/${params.slug}`}
        headline={`${playbook.name} — NDA English playbook`}
        description={playbook.summary}
      />
      <GuideHero
        eyebrow="Playbook"
        ...
        <>
          ...
          <section className="mt-12">
            ...
            <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
              The rules and habits that decide whether you get this question
              right.
            </p>
          ...
          {examples.length > 0 && (
            <section className="mt-12">
              ...
              <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
                Real past-year questions tagged to this playbook. Click to
          ...
          {detail.traps.length > 0 && (
            <section className="mt-12">
              ...
              <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
                Distractor shapes specific to this playbook. The page-wide{" "}
                <a className="text-primary hover:underline" href="/guide/nda-english/traps">
                  Traps section
                </a>{" "}

...
export async function generateMetadata({
  ...
  return {
    title: `${playbook.name} — NDA Physics playbook`,
    description: playbook.summary,
    alternates: { canonical: `/guide/nda-physics/playbooks/${params.slug}` },
  };
}
...
export default async function PlaybookDetail({ params }: { params: Params }) {
  ...
  const [taxonomy, examples] = await Promise.all([
    resolveTaxonomy(supabase, "NDA", "Physics"),
    detail
  ...
  const prev =
    idx > 0
      ? {
          href: `/guide/nda-physics/playbooks/${PLAYBOOKS[idx - 1].slug}`,
          label: PLAYBOOKS[idx - 1].name,
        }
      : { href: "/guide/nda-physics/playbooks", label: "All playbooks" };
  const next =
  ...
  return (
    <GuideShell
      ...
      breadcrumbs={[
        ...
        { href: "/guide/nda-physics/playbooks", label: "Playbooks" },
        { label: playbook.name },
      ]}
    >
      <GuideJsonLd
        type="Article"
        path={`/guide/nda-physics/playbooks/${params.slug}`}
        headline={`${playbook.name} — NDA Physics playbook`}
        description={playbook.summary}
      />
      <GuideHero
        eyebrow="Playbook"
          ...
          {detail.traps.length > 0 && (
            <section className="mt-12">
              ...
              <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
                ...
                <a className="text-primary hover:underline" href="/guide/nda-physics/traps">
                  Traps section
                </a>{" "}

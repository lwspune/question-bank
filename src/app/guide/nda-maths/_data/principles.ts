 ...
 *
 * `TOP_20` (now `TOP_11` post-2026-05-17 strict-cross-chapter prune): the
 * cross-chapter principles with deep-dive pages at /guide/nda-maths/principles/{slug}.
 * Each has a `slug` and is backed by DB tags in `question_principle_tags`
 * (migration 0023). Live counts + drill set come from the tag table at
 * request time via `loadPrincipleQuestionIds(slug)` — no static qCount,
 * no curated `extraQuestionIds`, no `drill[]` here.
 *
...
export type Principle = {
   ...
   *  For TOP_20 (slugged) principles the count comes from DB tags at request
   ...
   *  principles. TOP_20 principles use `?principle=<slug>` instead. */
...
/** All principles grouped by domain. TOP_20 entries appear in their domain

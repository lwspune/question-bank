...
import RelatedBiologyPlaybooks from "@/app/guide/_components/RelatedBiologyPlaybooks";
...
export async function generateMetadata({
  ...
  return {
    title: `${playbook.name} — NDA Biology playbook`,
    description: playbook.summary,
    alternates: { canonical: `/guide/nda-biology/playbooks/${params.slug}` },
  };
}
...
export default async function PlaybookDetail({ params }: { params: Params }) {
  ...
  const [taxonomy, examples] = await Promise.all([
    resolveTaxonomy(supabase, "NDA", "Biology"),
    detail
  ...
  const prev =
    idx > 0
      ? {
          href: `/guide/nda-biology/playbooks/${PLAYBOOKS[idx - 1].slug}`,
          label: PLAYBOOKS[idx - 1].name,
        }
      : { href: "/guide/nda-biology/playbooks", label: "All playbooks" };
  const next =
  ...
  return (
    <GuideShell
      ...
      breadcrumbs={[
        ...
        { href: "/guide/nda-biology/playbooks", label: "Playbooks" },
        { label: playbook.name },
      ]}
    >
      <GuideJsonLd
        type="Article"
        path={`/guide/nda-biology/playbooks/${params.slug}`}
        headline={`${playbook.name} — NDA Biology playbook`}
        description={playbook.summary}
      />
      <GuideHero
        eyebrow="Playbook"
          ...
          {detail.traps.length > 0 && (
            <section className="mt-12">
              ...
              <p className="mt-2 font-serif text-sm leading-relaxed text-muted-foreground">
                ...
                <a className="text-primary hover:underline" href="/guide/nda-biology/traps">
                  Traps section
                </a>{" "}
      ...
      {detail && <RelatedBiologyPlaybooks slugs={detail.relatedSlugs} />}

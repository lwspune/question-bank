...
import { CHAPTER_TABLE, OVERVIEW, ROUTES } from "./_data/nda-chemistry";
...
export default async function NdaChemistryLanding() {
  ...
  return (
    ...
    >
      ...
      <GuideHero
        ...
        subtitle={`A ${OVERVIEW.totalQ.toLocaleString("en-IN")}-question analysis of the Chemistry half of GAT PART B from 2017 to 2026. We mapped the 12 chapter playbooks, the 50 most-tested chemical compounds, the year-on-year drift (the paper has NOT hardened), and the distractor traps — so you can prep what the exam actually tests.`}
      >
      ...
      <BrowseLink
        ...
        className="mt-2"
      >
        Browse the full NDA Chemistry bank
      </BrowseLink>
      ...
      <section className="mt-12">
        ...
        <p className="mt-2 max-w-2xl font-serif text-sm leading-relaxed text-muted-foreground sm:text-base">
          All 12 chapters tested in NDA PART B Chemistry, sized by question
      ...
      <section className="mt-12">
        ...
        <ul className="mt-6 grid gap-3 sm:grid-cols-2">
          {sectionCards.map((r) => (
            <li key={r.slug}>
              <Link
                href={`/guide/nda-chemistry/${r.slug}`}
      ...
      <section className="mt-12 rounded-lg border bg-muted/30 p-5">
        ...
        <div className="mt-2 space-y-2 font-serif text-sm leading-relaxed text-muted-foreground">
          <p>
            Most NDA Chemistry prep splits two ways: a textbook chapter-list
      ...
      <PrevNextNav
        next={{
          href: "/guide/nda-chemistry/strategy",
          label: "Strategy — Recall, Rule, Calculate",
        }}
      />
    </GuideShell>
  );

...
const BUCKET_LABEL = {
  recall:
    "Recall traps (Indian Geography Economy · Indian Geography Physical · World/Human)",
  apply: "Apply traps (Climatology · Earth's Structure · Oceanography mechanism flips)",
  verify: "Verify traps (Earth in Space · Oceanography · multi-statement everywhere)",
} as const;
...
export default async function Traps() {
  ...
  return (
    ...
    >
      <GuideJsonLd
        type="Article"
        path="/guide/nda-geography/traps"
        headline="NDA Geography Traps — Distractor patterns NDA reuses"
        description="How candidates who know geography still lose marks. Mineral-state swap, river-state misalignment, peak-range swap, Coriolis-direction flip, warm/cold-current swap, rock-classification error."
      />
      <GuideHero
        eyebrow="Traps"
        title="How NDA loses you marks even when you know the geography"
        subtitle="Geography distractors are a MIX — paired-fact swap (state↔river, mineral↔state, peak↔range, crop↔season) dominates Recall; mechanism-direction flips (Coriolis direction, current temperature, plate-boundary outcome) dominate Apply; multi-statement partial-credit + universal-claim traps dominate Verify. Different from Chemistry (compound identity), Physics (formula misapplication), Biology (paired-fact-recall only). Each trap below is illustrated on a real PYQ where one exists."
      >
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      ...
                      {trap.affects.map((slug, i) => (
                        <span key={slug}>
                          <Link
                            href={`/guide/nda-geography/playbooks/${slug}`}
      ...
      <section className="mt-14 rounded-lg border-2 border-primary/40 bg-primary/5 p-6">
        ...
        <p className="mt-2 font-serif leading-relaxed text-muted-foreground">
          Geography&rsquo;s verification habit is different from Physics &mdash; there&rsquo;s
          no unit-check or sign-check (mostly). The Recall lever is paired-fact recall (river
          ↔ state; mineral ↔ producer state; peak ↔ range). The Apply lever is mechanism-direction
          (Coriolis right N / left S; cold current toward equator on west coast). The Verify
          lever is statement-by-statement T/F. Match the tier to the question shape.
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-md border bg-card p-4">
            ...
            <p className="mt-1 font-serif text-xs leading-relaxed text-muted-foreground">
              Iron ore → Odisha. Coal → Jharkhand. Mahendragiri → Odisha (Eastern Ghats).
              Anamudi → Kerala (Western Ghats). State both halves of the pair before picking.
            </p>
          </div>
          <div className="rounded-md border bg-card p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <p className="mt-1 text-sm font-semibold tracking-tight">
              Mechanism direction trace
            </p>
            <p className="mt-1 font-serif text-xs leading-relaxed text-muted-foreground">
              Coriolis → right N / left S. Cold currents → poles to equator on W coasts.
              Plate boundary → write the type (convergent o-c / c-c / o-o / divergent / transform).
              Rock cycle → identify the formation process (cooled magma / compacted sediments / heat+pressure).
            </p>
          </div>
          <div className="rounded-md border bg-card p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <p className="mt-1 font-serif text-xs leading-relaxed text-muted-foreground">
              ...
              question uncertain. Watch for universal claims (all / every / always).
            </p>
          </div>
        </div>
        <p className="mt-4 font-serif text-sm leading-relaxed text-muted-foreground">
          <strong className="font-semibold text-foreground">
            The 10-second pair-check is the highest-leverage habit on Recall;
            the mechanism-direction trace on Apply.
          </strong>{" "}
          Most Geography distractors fall to one of these. A guess at 5 seconds
          without either is negative-EV; a 10-second check + skip is strictly better.
        </p>
      </section>

      <PrevNextNav
        prev={{ href: "/guide/nda-geography/trends", label: "Trends" }}
        next={{ href: "/guide/nda-geography", label: "Back to Overview" }}
      />
    </GuideShell>
  );
}

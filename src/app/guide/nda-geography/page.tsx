...
import { CHAPTER_TABLE, OVERVIEW, ROUTES } from "./_data/nda-geography";
...
export default async function NdaGeographyLanding() {
  ...
  return (
    ...
    >
      ...
      <GuideHero
        ...
        subtitle={`A ${OVERVIEW.totalQ.toLocaleString("en-IN")}-question analysis of the Geography section of GAT PART A from 2017 to 2026. We mapped the 7 chapter playbooks, the 62 most-tested named-fact pairs, the year-on-year drift (the paper has NOT consistently hardened — 2018 + 2025 were outlier highs), and the distractor traps — so you can prep what the exam actually tests.`}
      >
      ...
      <BrowseLink
        ...
        className="mt-2"
      >
        Browse the full NDA Geography bank
      </BrowseLink>
      ...
      <section className="mt-12">
        ...
        <p className="mt-2 max-w-2xl font-serif text-sm leading-relaxed text-muted-foreground sm:text-base">
          All 7 chapters tested in NDA PART A Geography, sized by question
      ...
      <section className="mt-12">
        ...
        <ul className="mt-6 grid gap-3 sm:grid-cols-2">
          {sectionCards.map((r) => (
            <li key={r.slug}>
              <Link
                href={`/guide/nda-geography/${r.slug}`}
      ...
      <section className="mt-12 rounded-lg border bg-muted/30 p-5">
        ...
        <div className="mt-2 space-y-2 font-serif text-sm leading-relaxed text-muted-foreground">
          <p>
            Most NDA Geography prep splits two ways: a textbook chapter-list
      ...
      <PrevNextNav
        next={{
          href: "/guide/nda-geography/strategy",

...
import { listReports } from "@/lib/reports/listReports";
import {
...
export default async function ReportsTriagePage({
  ...
  const reports = await listReports(supabase, { status, limit: 100 });
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Reports filed against questions owned by {member.orgName}. Resolve,
            mark won&apos;t-fix, or open the question to fix the underlying
            issue.
          </p>
        </div>
        ...
        <nav aria-label="Status filter" className="flex flex-wrap gap-1.5">
          {STATUS_OPTIONS.map((opt) => {
            ...
            const href =
              opt.value === "open"
                ? "/dashboard/reports"
                : `/dashboard/reports?status=${opt.value}`;
            return (
              <Link
                key={opt.value}
                href={href}
        ...
        {reports.length === 0 ? (
          <div className="rounded-lg border border-dashed bg-muted/30 p-10 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              When users report questions, they&apos;ll show up here for triage.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t pt-3 text-xs text-muted-foreground">
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                    <span>
                      Filed {new Date(r.createdAt).toLocaleString("en-IN", {
                        dateStyle: "medium",
                        timeStyle: "short",
                      })}
                    </span>
...
function categoryChipClass(category: ReportCategory): string {
  // Light visual differentiation by category. Severity-ish gradient.
  const danger =
